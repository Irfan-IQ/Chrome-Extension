...
async function getSession(): Promise<{ session: ort.InferenceSession; loadMs: number }> {
  if (!MODEL_PATH) {
    throw new Error("YuNet model URL has not been configured.");
  }
  if (session) {
...
self.addEventListener("message", async (event: MessageEvent) => {
  ...
  if (message.type === "init") {
    MODEL_PATH = message.modelUrl || null;
    try {
      const { loadMs } = await getSession();
      self.postMessage({ type: "ready", ok: true, loadMs, modelUrl: MODEL_PATH });
    } catch (error) {
      self.postMessage({
        type: "ready",
        ok: false,
  ...
  if (message.type !== "detect" || !message.image || typeof message.id !== "number") {
    return;
  }
  ...
  try {
    ...
    const { tensor, scale, offsetX, offsetY } = createInputTensor(image);
